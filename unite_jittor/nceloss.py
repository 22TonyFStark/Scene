...
class BidirectionalNCE1(nn.Module):
    ...
    def execute(self, feat_q, feat_k):
        ...
        # TODO: 有点不一样，这里torch是自带广播的，而jittor需要repeat成batchsize个
        ...
        # TODO: 这里原本是torch.long，但jittor用long会报错，改成‘int32’，注意这里是字符串
        loss = nn.cross_entropy_loss(out, jittor.zeros(out.size(0), dtype='int32'),reduction='none')
...
class BidirectionalNCE2(nn.Module):
    ...
    def execute(self, feat_q, feat_k):
        ...
        loss = self.cross_entropy_loss(out, jittor.zeros(out.size(0), dtype='int32',
...
class SRNCE(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        # TODO check this out: reduction may be wrong
        # self.cross_entropy_loss = nn.CrossEntropyLoss(reduction='none')
        self.mask_dtype = 'bool'  #torch.bool
        self.l2_norm = Normalize(2)
    ...
    def execute(self, feat_q, feat_k, feat_c):
        ...
        loss1 = nn.cross_entropy_loss(out1, jittor.zeros(out1.size(0), dtype='int32',
                                                          device=feat_q.device),reduction='none')
        out2 = l_pos2 / self.args.nce_T

        loss2 = nn.cross_entropy_loss(out2, jittor.zeros(out2.size(0), dtype='int32',
                                                          device=feat_q.device),reduction='none')

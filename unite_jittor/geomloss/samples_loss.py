...
class SamplesLoss(jittor.Module):
    ...
    def generate_weights(self, x):
        if x.ndim == 2:  # 
            N = x.shape[0]
            return jittor.ones(N).type_as(x) / N
        elif x.ndim == 3:
            B, N, _ = x.shape
            return jittor.ones(B,N).type_as(x) / N
        else:
...
if __name__ == '__main__':
    ...
    a = jittor.randn([6,4096])
    b = jittor.randn([6,4096, 2304])
    c = jittor.randn([6,4096])
    d = jittor.randn([6,4096, 2304])
    F,G = uot(a,b,c,d)
    print('finish')
...
class PatchSampleF(nn.Module):
    ...
    def execute(self, feat, num_patches=64, patch_ids=None):
        ...
        if patch_ids is not None:
            ...
            patch_id = jittor.randperm(feat_reshape.shape[1], device=feat[0].device)
...
class NoVGGCorrespondence(BaseNetwork):
    ...
    def execute(self, ref_img, real_img, seg_map, ref_seg_map, detach_flag=False):
        ...
        coor_out['weight1'] = nn.interpolate(theta_w_, size=(256, 256))
        coor_out['weight2'] = nn.interpolate(phi_w_, size=(256, 256))
